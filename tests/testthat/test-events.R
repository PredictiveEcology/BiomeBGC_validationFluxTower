...
buildMonthlyBBGC <- function() {
  data.table::data.table(
    ...
    daily_gr = 0.0003, daily_hr = 0.0004,
...
buildAnnualBBGC <- function() {
  data.table::data.table(
    ...
    daily_mr = c(0.0006, 0.0007), daily_gr = 0.0003, daily_hr = 0.0004,
...
runValidationSim <- function(dailyOutput, monthlyAverages, annualAverages, outputPath) {
  dir.create(file.path(outputPath, moduleName), recursive = TRUE, showWarnings = FALSE)
  simInit(
...
test_that("the module computes NEE validation metrics at daily/monthly/yearly scales", {
  outputPath <- file.path(tempdir(), paste0("validationFluxTower_nee_", sample.int(1e6, 1)))
  sim <- runValidationSim(buildDailyBBGC(), buildMonthlyBBGC(), buildAnnualBBGC(), outputPath)

  expect_s4_class(sim, "simList")
  vs <- sim$validationSummary
  expect_true(is.data.frame(vs))
  expect_setequal(vs$timescale[vs$estimate == "NEE"], c("daily", "month", "year"))

  # Independently recompute the daily NEE metrics from the same inputs and compare
  dayComparison <- mergeData(towerDaily, buildDailyBBGC(), timescale = "day", outputVar = "NEE") |> na.omit()
  ...
  expect_equal(dailyRow$R2, cor(dayComparison$BBGC, dayComparison$fluxTower)^2, tolerance = 1e-9)
})

test_that("the module computes RECO and GPP validation metrics from real 2004 tower data", {
  outputPath <- file.path(tempdir(), paste0("validationFluxTower_recogpp_", sample.int(1e6, 1)))
  sim <- runValidationSim(buildDailyBBGC(), buildMonthlyBBGC(), buildAnnualBBGC(), outputPath)
  ...
  expect_setequal(vs$timescale[vs$estimate == "RECO"], c("daily", "month", "year"))
  expect_setequal(vs$timescale[vs$estimate == "GPP"], c("daily", "month", "year"))

  # Independently recompute the daily RECO metrics from the same inputs and compare
  recoDay <- mergeData(towerDaily, buildDailyBBGC(), timescale = "day", outputVar = "RECO") |> na.omit()
  ...
  gppDay <- mergeData(towerDaily, buildDailyBBGC(), timescale = "day", outputVar = "GPP") |> na.omit()
...
test_that("the module writes validationSummary.csv on the save event", {
  outputPath <- file.path(tempdir(), paste0("validationFluxTower_save_", sample.int(1e6, 1)))
  runValidationSim(buildDailyBBGC(), buildMonthlyBBGC(), buildAnnualBBGC(), outputPath)
...
test_that("compareRECO and compareGPP are skipped gracefully when their BiomeBGC outputs are absent", {
  outputPath <- file.path(tempdir(), paste0("validationFluxTower_nogpp_", sample.int(1e6, 1)))

  expect_message(
    sim <- runValidationSim(buildDailyBBGCNoPartitioning(), buildMonthlyBBGC(), buildAnnualBBGC(), outputPath),
    "skipped"
  )
